...
╠══════╬══════════════╬════════════════════════════════════════════════════════════╣
║ 0x01 ║ WRITE_REG    ║ Cho phép ghi dữ liệu 32-bit vào các thanh ghi CTRL, tWD,   ║
║      ║              ║ tRST hoặc arm_delay. Cần gửi kèm 4 byte dữ liệu.           ║
╟──────╫──────────────╫────────────────────────────────────────────────────────────╢
║ 0x02 ║ READ_REG     ║ Trả về giá trị 32-bit của thanh ghi tại địa chỉ yêu cầu.   ║
║      ║              ║ FPGA sẽ phản hồi dữ liệu qua khối uart_tx                  ║
╟──────╫──────────────╫────────────────────────────────────────────────────────────╢
║ 0x03 ║ KICK         ║ Giả lập sự kiện nút nhấn S1 bằng phần mềm. Lệnh này không  ║
║      ║              ║ cần kèm theo dữ liệu (LEN = 0)                             ║
╟──────╫──────────────╫────────────────────────────────────────────────────────────╢
║ 0x04 ║ GET_STATUS   ║ Đọc trực tiếp nội dung thanh ghi STATUS (0x10) để kiểm tra ║
║      ║              ║ trạng thái lỗi, nguồn kick và chế độ enable                ║
╚══════╩══════════════╩════════════════════════════════════════════════════════════╝

Một số ví dụ các câu lệnh UART
...
3. Thêm mã nguồn: Thêm toàn bộ 8 file .v bao gồm: top.v, watchdog_core.v, regfile.v, uart.v, uart_rx.v, uart_tx.v, uart_engine.v và debounce.v.
4. Thêm file ràng buộc: - Thêm wdt.cst làm tệp Physical Constraints (để định nghĩa chân nút nhấn, LED, UART).4.
     Thêm wdt.sdc làm tệp Timing Constraints (để định nghĩa xung nhịp 27MHz).
5. Lưu ý về mô phỏng: Không thêm file testbench vào project khi thực hiện tổng hợp phần cứng.
6. Cấu hình chân nạp: Vào Project -> Configuration -> Dual-Purpose Pin: bật tùy chọn "Use JTAG as regular IO" để tránh xung đột chân UART (đặc biệt quan trọng trên chip 1P5).
7. Biên dịch: Thực hiện trình tự Synthesize -> Place & Route -> Gowin Programmer
